...
export default function TaskItem({ task, onToggle, onDelete, onEdit }: TaskItemProps) {
  ...
  const getRecurrenceIcon = () => {
    ...
    return recurrenceIcons[task.recurrencePattern || 'daily'] || '🔄';
  };

  return (
    <li className="py-4 px-2 hover:bg-white/10 rounded-lg transition-colors bg-red-200">
        ...
        <div className="flex items-start space-x-3 flex-1 min-w-0">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              ...
              {task.isRecurring && (
                <span className="text-sm" title={`Recurring ${task.recurrencePattern}`}>
                  {getRecurrenceIcon()}
                </span>
              )}
            </div>
            ...
            <div className="mt-2 flex flex-wrap gap-2 text-xs">
              ...
              {task.tags && task.tags.split(',').map((tag, index) => (
                <span key={index} className="bg-gray-500/10 text-gray-500 px-2 py-1 rounded">
                  #{tag.trim()}
                </span>
              ))}
